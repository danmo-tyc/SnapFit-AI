            ...
            <div className={`${isMobile ? 'flex flex-col space-y-3' : 'flex justify-between items-center'}`}>
              ...
              {!isMobile && (
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <CardTitle className="text-lg">{currentExpert.name}</CardTitle>
                    <p className="text-sm text-muted-foreground">{currentExpert.title}</p>
                  </div>
                </div>
              )}
              ...
              <div className={`${isMobile ? 'flex flex-col space-y-2' : 'flex items-center space-x-4'}`}>
                <div className={`flex items-center ${isMobile ? 'justify-between' : 'space-x-2'}`}>
                  ...
                  <Label htmlFor="include-data" className={`${isMobile ? 'text-sm' : ''}`}>包含健康数据</Label>
                </div>
          ...
          <ScrollArea className={`flex-1 w-full ${isMobile ? 'pr-2' : 'pr-4'}`}>
            <div className={`space-y-3 pb-4 w-full max-w-full overflow-hidden ${isMobile ? 'space-y-2' : 'space-y-4'}`}>
              {!isClient ? (
                ...
                <div className={`text-center ${isMobile ? 'py-4' : 'py-8'}`}>
                  <p className={`font-medium ${isMobile ? 'text-base' : 'text-lg'}`}>欢迎使用智能健康助手</p>
                  <p className={`text-muted-foreground ${isMobile ? 'text-sm mt-1' : 'mt-2'}`}>您可以询问任何健康、饮食或运动相关的问题</p>
                  {!checkAIConfig() && <p className="text-sm text-amber-600 mt-2">请先在设置页面配置聊天模型</p>}
                </div>
              ) : (
